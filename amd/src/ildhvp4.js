define([
  ...
  ILD.setResult = function (contentid, score, maxScore) {
    var promises = ajax.call([
      {
        methodname: "local_ildhvp4_setgrade",
  ...
  ILD.showModal = async function (data) {
    if (data.show_chapter_modal) {
      let navdrawerChapter = window.parent.document.querySelector(
      ...
      if (data.next_chapter < 0) {
        ...
        const modal = await window.ModalFactory.create({
